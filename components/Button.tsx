...
const Button = ({text, color, icon, link}: {text: string, color: string, icon?: React.ReactNode, link: string}) => {
  return (
    <div  
        className={`${color == "gray" ? "border border-gray-500 bg-black" : "bg-purple-700"} ${icon && "w-[140px]"} z-0 overflow-hidden h-[35px] w-[120px] flex items-center justify-center rounded-md font-semibold group text-sm`}
    >
